...
void
new_route (Route *routes, const char *path, Handler handler)
{
  Route route;
...
enum MHD_Result
router (Route *routes, const RequestContext *req_ctx)
{
  char *response;
  size_t url_len;
  ...
  if (url_len > MAX_ROUTE_LEN)
    {
      required_len
          = snprintf (NULL, 0, ERR_TEMPLATE, MHD_HTTP_BAD_REQUEST,
      ...
      return send_response_free_callback (req_ctx->connection, error,
                                          required_len, MHD_HTTP_BAD_REQUEST,
                                          &mem_free);
    }
  ...
  if (!routes[id].path)
    {
      ...
      return send_response_free_callback (req_ctx->connection, error,
                                          required_len, MHD_HTTP_NOT_FOUND,
                                          &mem_free);
    }
